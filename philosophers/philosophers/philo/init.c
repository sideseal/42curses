...
/*   Updated: 2023/04/15 01:01:03 by gychoi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_shared(t_shared *shared, t_param param)
{
	...
	while (++i < param.philo_num)
		if (pthread_mutex_init(&(shared->forks_mutex)[i], 0) \
			|| pthread_mutex_init(&(shared->philo_mutex)[i], 0))
			return (clear_shared_mutex(shared, i)); // 고민해볼 것
	if (pthread_mutex_init(&(shared->shared_mutex), 0))
		return (clear_shared_mutex(shared, i)); // clear 뮤텍스 여러개 해야 함
...
int	init_philos(t_shared *shared, t_param param, t_philo **philos)
{
	...
	while (++i < param.philo_num)
	{
		...
		(*philos)[i].philo_count_eat = param.philo_must_eat + 1;
